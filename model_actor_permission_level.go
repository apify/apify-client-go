...
// ActorPermissionLevel Determines permissions that the Actor requires to run. For more information, see the [Actor permissions documentation](https://docs.apify.com/platform/actors/development/permissions). 
type ActorPermissionLevel string

// List of ActorPermissionLevel
const (
	LIMITED_PERMISSIONS ActorPermissionLevel = "LIMITED_PERMISSIONS"
	FULL_PERMISSIONS ActorPermissionLevel = "FULL_PERMISSIONS"
)

// All allowed values of ActorPermissionLevel enum
var AllowedActorPermissionLevelEnumValues = []ActorPermissionLevel{
	"LIMITED_PERMISSIONS",
	"FULL_PERMISSIONS",
}

func (v *ActorPermissionLevel) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := ActorPermissionLevel(value)
	for _, existing := range AllowedActorPermissionLevelEnumValues {
	...
	return fmt.Errorf("%+v is not a valid ActorPermissionLevel", value)
}

// NewActorPermissionLevelFromValue returns a pointer to a valid ActorPermissionLevel
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewActorPermissionLevelFromValue(v string) (*ActorPermissionLevel, error) {
	ev := ActorPermissionLevel(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for ActorPermissionLevel: valid values are %v", v, AllowedActorPermissionLevelEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v ActorPermissionLevel) IsValid() bool {
	for _, existing := range AllowedActorPermissionLevelEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to ActorPermissionLevel value
func (v ActorPermissionLevel) Ptr() *ActorPermissionLevel {
	return &v
}

type NullableActorPermissionLevel struct {
	value *ActorPermissionLevel
	isSet bool
}

func (v NullableActorPermissionLevel) Get() *ActorPermissionLevel {
	return v.value
}

func (v *NullableActorPermissionLevel) Set(val *ActorPermissionLevel) {
	v.value = val
	v.isSet = true
}

func (v NullableActorPermissionLevel) IsSet() bool {
	return v.isSet
}

func (v *NullableActorPermissionLevel) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableActorPermissionLevel(val *ActorPermissionLevel) *NullableActorPermissionLevel {
	return &NullableActorPermissionLevel{value: val, isSet: true}
}

func (v NullableActorPermissionLevel) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableActorPermissionLevel) UnmarshalJSON(src []byte) error {
	v.isSet = true
