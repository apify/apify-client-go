...
`MonthlyUsage` reports usage for the current month and returns the raw JSON usage report from
the API — a JSON object with the account's usage breakdown and totals for the period.
