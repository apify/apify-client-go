...
// StorageOwnership the model 'StorageOwnership'
type StorageOwnership string

// List of StorageOwnership
const (
	OWNED_BY_ME StorageOwnership = "ownedByMe"
	SHARED_WITH_ME StorageOwnership = "sharedWithMe"
)

// All allowed values of StorageOwnership enum
var AllowedStorageOwnershipEnumValues = []StorageOwnership{
	"ownedByMe",
	"sharedWithMe",
}

func (v *StorageOwnership) UnmarshalJSON(src []byte) error {
	var value string
	err := json.Unmarshal(src, &value)
	if err != nil {
		return err
	}
	enumTypeValue := StorageOwnership(value)
	for _, existing := range AllowedStorageOwnershipEnumValues {
	...
	return fmt.Errorf("%+v is not a valid StorageOwnership", value)
}

// NewStorageOwnershipFromValue returns a pointer to a valid StorageOwnership
// for the value passed as argument, or an error if the value passed is not allowed by the enum
func NewStorageOwnershipFromValue(v string) (*StorageOwnership, error) {
	ev := StorageOwnership(v)
	if ev.IsValid() {
		return &ev, nil
	} else {
		return nil, fmt.Errorf("invalid value '%v' for StorageOwnership: valid values are %v", v, AllowedStorageOwnershipEnumValues)
	}
}

// IsValid return true if the value is valid for the enum, false otherwise
func (v StorageOwnership) IsValid() bool {
	for _, existing := range AllowedStorageOwnershipEnumValues {
		if existing == v {
			return true
		}
...
// Ptr returns reference to StorageOwnership value
func (v StorageOwnership) Ptr() *StorageOwnership {
	return &v
}

type NullableStorageOwnership struct {
	value *StorageOwnership
	isSet bool
}

func (v NullableStorageOwnership) Get() *StorageOwnership {
	return v.value
}

func (v *NullableStorageOwnership) Set(val *StorageOwnership) {
	v.value = val
	v.isSet = true
}

func (v NullableStorageOwnership) IsSet() bool {
	return v.isSet
}

func (v *NullableStorageOwnership) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableStorageOwnership(val *StorageOwnership) *NullableStorageOwnership {
	return &NullableStorageOwnership{value: val, isSet: true}
}

func (v NullableStorageOwnership) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableStorageOwnership) UnmarshalJSON(src []byte) error {
	v.isSet = true
