...
// LastActorRunsRebootAPIService LastActorRunsRebootAPI service
type LastActorRunsRebootAPIService service

type ApiActRunsLastRebootPostRequest struct {
	ctx context.Context
	ApiService *LastActorRunsRebootAPIService
	actorId string
	status *string
}

// Filter for the run status.
func (r ApiActRunsLastRebootPostRequest) Status(status string) ApiActRunsLastRebootPostRequest {
	r.status = &status
	return r
}

func (r ApiActRunsLastRebootPostRequest) Execute() (*RunResponse, *http.Response, error) {
	return r.ApiService.ActRunsLastRebootPostExecute(r)
}

/*
ActRunsLastRebootPost Reboot Actor's last run
...
or request queue will be lost.
 ...
 @return ApiActRunsLastRebootPostRequest
*/
func (a *LastActorRunsRebootAPIService) ActRunsLastRebootPost(ctx context.Context, actorId string) ApiActRunsLastRebootPostRequest {
	return ApiActRunsLastRebootPostRequest{
		ApiService: a,
		ctx: ctx,
		actorId: actorId,
...
//  @return RunResponse
func (a *LastActorRunsRebootAPIService) ActRunsLastRebootPostExecute(r ApiActRunsLastRebootPostRequest) (*RunResponse, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "LastActorRunsRebootAPIService.ActRunsLastRebootPost")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v2/actors/{actorId}/runs/last/reboot"
