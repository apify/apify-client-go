...
func (r ApiLogGetRequest) Stream(stream bool) ApiLogGetRequest {
	r.stream = &stream
	return r
}

// If &#x60;true&#x60; or &#x60;1&#x60; then the web browser will download the log file rather than open it in a tab. 
func (r ApiLogGetRequest) Download(download bool) ApiLogGetRequest {
	r.download = &download
	return r
}
...
func (r ApiLogGetRequest) Raw(raw bool) ApiLogGetRequest {
	r.raw = &raw
	return r
}

func (r ApiLogGetRequest) Execute() (string, *http.Response, error) {
...
Retrieves logs for a specific Actor build or run.
 ...
 @param buildOrRunId ID of the Actor build or run.
 @return ApiLogGetRequest
*/
func (a *LogsAPIService) LogGet(ctx context.Context, buildOrRunId string) ApiLogGetRequest {
	return ApiLogGetRequest{
		ApiService: a,
		ctx: ctx,
...
//  @return string
func (a *LogsAPIService) LogGetExecute(r ApiLogGetRequest) (string, *http.Response, error) {
	var (
	...
	localBasePath, err := a.client.cfg.ServerURLWithContext(r.ctx, "LogsAPIService.LogGet")
	if err != nil {
		return localVarReturnValue, nil, &GenericOpenAPIError{error: err.Error()}
	}

	localVarPath := localBasePath + "/v2/logs/{buildOrRunId}"
	localVarPath = strings.Replace(localVarPath, "{"+"buildOrRunId"+"}", url.PathEscape(parameterValueToString(r.buildOrRunId, "buildOrRunId")), -1)
