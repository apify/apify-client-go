...
// DeletedRequest Confirmation of a request that was successfully deleted from a request queue.
...
func (dst *DeletedRequest) UnmarshalJSON(data []byte) error {
	var err error
	// try to unmarshal JSON data into DeletedRequestById
	err = json.Unmarshal(data, &dst.DeletedRequestById);
	if err == nil {
		jsonDeletedRequestById, _ := json.Marshal(dst.DeletedRequestById)
		if string(jsonDeletedRequestById) == "{}" { // empty struct
			dst.DeletedRequestById = nil
		} else {
			return nil // data stored in dst.DeletedRequestById, return on the first match
		}
	} else {
		dst.DeletedRequestById = nil
	}

	// try to unmarshal JSON data into DeletedRequestByUniqueKey
	err = json.Unmarshal(data, &dst.DeletedRequestByUniqueKey);
	if err == nil {
		jsonDeletedRequestByUniqueKey, _ := json.Marshal(dst.DeletedRequestByUniqueKey)
		if string(jsonDeletedRequestByUniqueKey) == "{}" { // empty struct
			dst.DeletedRequestByUniqueKey = nil
		} else {
			return nil // data stored in dst.DeletedRequestByUniqueKey, return on the first match
		}
	} else {
		dst.DeletedRequestByUniqueKey = nil
	}

	return fmt.Errorf("data failed to match schemas in anyOf(DeletedRequest)")
}

// Marshal data from the first non-nil pointers in the struct to JSON
func (src DeletedRequest) MarshalJSON() ([]byte, error) {
	if src.DeletedRequestById != nil {
		return json.Marshal(&src.DeletedRequestById)
	}

	if src.DeletedRequestByUniqueKey != nil {
		return json.Marshal(&src.DeletedRequestByUniqueKey)
	}
...
type NullableDeletedRequest struct {
	value *DeletedRequest
	isSet bool
}

func (v NullableDeletedRequest) Get() *DeletedRequest {
	return v.value
}

func (v *NullableDeletedRequest) Set(val *DeletedRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableDeletedRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableDeletedRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableDeletedRequest(val *DeletedRequest) *NullableDeletedRequest {
	return &NullableDeletedRequest{value: val, isSet: true}
}

func (v NullableDeletedRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableDeletedRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
